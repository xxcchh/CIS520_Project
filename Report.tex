...
Our final method which gotr 81.82\% accuracy on the leaderboard is achieved in this way. Firstly, from the testing on trainning dataset, we found that Sentiment Analysis has a high accuracy predicting those negative tweets. So the intuition was really straightfoward, if the test data calculated by Sentiment Analysis 1 has negative words and don't have positive words, we will label it as negative. Then how we deal with those Sentiment Analysis can't predict? Because Naive Bayes and SVM and Gentleboost all have a quite good accuracy on test data, so we use the combine of the three methods throught mojority vote to predict them.\\\\
